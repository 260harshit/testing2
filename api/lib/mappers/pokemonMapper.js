...
function mapPokemonDataToPokemonGraphQL({
  ...
  const resolvedRecursingAs = recursingAs ? `${recursingAs}.` : "";
  const genderEntryRequestedFields = cast(requestedFields);
  const genderObject = {};
  const basePokemonGenderRatio = data.genderRatio || {
    male: "50%",
    female: "50%"
  };
  ...
  const baseStatsRequestedFields = cast(requestedFields);
  ...
  const evYieldsRequestedFields = cast(requestedFields);
  const evYieldsObject = {};
  addPropertyToObjectFieldBased({
  ...
  const abilitiesRequestedFields = requestedFields.filterStartsWith(`${resolvedRecursingAs}abilities.`);
  addPropertyToObjectFieldBased({
    objectTarget: abilitiesObject,
    propertyKey: "first",
    propertyValue: () => resolvedNestedAbility(
      data.abilities.first,
      abilitiesRequestedFields.filterStartsWith(`${resolvedRecursingAs}abilities.first.`, true)
    ),
  ...
  addPropertyToObjectFieldBased({
    ...
    propertyValue: () => resolvedNestedAbility(
      ...
      abilitiesRequestedFields.filterStartsWith(`${resolvedRecursingAs}abilities.second.`, true)
    ),
    requestedFields: abilitiesRequestedFields,
  ...
  addPropertyToObjectFieldBased({
    ...
    propertyValue: () => resolvedNestedAbility(
      ...
      abilitiesRequestedFields.filterStartsWith(`${resolvedRecursingAs}abilities.hidden.`, true)
    ),
    requestedFields: abilitiesRequestedFields,
  ...
  addPropertyToObjectFieldBased({
    ...
    propertyValue: () => resolvedNestedAbility(
      ...
      abilitiesRequestedFields.filterStartsWith(`${resolvedRecursingAs}abilities.special.`, true)
    ),
    requestedFields: abilitiesRequestedFields,
    fieldAccessor: `${resolvedRecursingAs}abilities.special`
  });
  const pokemonTypesArray = [];
  const pokemonTypeRequestedFields = requestedFields.filterStartsWith(`${resolvedRecursingAs}types.`);
  for (const type of data.types) {
    ...
    addPropertyToObjectConditional({
      objectTarget: pokemonTypeObject,
      propertyKey: "matchup",
      propertyValue: () => resolveNestedType(type, pokemonTypeRequestedFields.filterStartsWith(`${resolvedRecursingAs}types.matchup.`, true)),
  ...
  const generationalLearnsetsObject = {};
  const generationalLearnsetsRequestedFields = requestedFields.filterStartsWith(
    `${resolvedRecursingAs}learnsets.`
  );
  addPropertyToObjectConditional({
    objectTarget: generationalLearnsetsObject,
    propertyKey: "generation3",
    propertyValue: () => resolvedNestedLearnset(
      parsingPokemon || toLowerSingleWordCase(data.species),
      3,
      generationalLearnsetsRequestedFields.filterStartsWith(`${resolvedRecursingAs}learnsets.generation3.`, true)
    ),
  ...
  addPropertyToObjectConditional({
    ...
    propertyValue: () => resolvedNestedLearnset(
      parsingPokemon || toLowerSingleWordCase(data.species),
      4,
      generationalLearnsetsRequestedFields.filterStartsWith(`${resolvedRecursingAs}learnsets.generation4.`, true)
    ),
  ...
  addPropertyToObjectConditional({
    ...
    propertyValue: () => resolvedNestedLearnset(
      parsingPokemon || toLowerSingleWordCase(data.species),
      5,
      generationalLearnsetsRequestedFields.filterStartsWith(`${resolvedRecursingAs}learnsets.generation5.`, true)
    ),
  ...
  addPropertyToObjectConditional({
    ...
    propertyValue: () => resolvedNestedLearnset(
      parsingPokemon || toLowerSingleWordCase(data.species),
      6,
      generationalLearnsetsRequestedFields.filterStartsWith(`${resolvedRecursingAs}learnsets.generation6.`, true)
    ),
  ...
  addPropertyToObjectConditional({
    ...
    propertyValue: () => resolvedNestedLearnset(
      parsingPokemon || toLowerSingleWordCase(data.species),
      7,
      generationalLearnsetsRequestedFields.filterStartsWith(`${resolvedRecursingAs}learnsets.generation7.`, true)
    ),
  ...
  addPropertyToObjectConditional({
    ...
    propertyValue: () => resolvedNestedLearnset(
      parsingPokemon || toLowerSingleWordCase(data.species),
      9,
      generationalLearnsetsRequestedFields.filterStartsWith(`${resolvedRecursingAs}learnsets.generation8.`, true)
    ),
    condition: generationalLearnsetsRequestedFields.hasStartsWith(`${resolvedRecursingAs}learnsets.generation8.`)
  });
  const catchRateRequestedFields = cast(requestedFields);
  const catchRateObject = {};
  const basePokemonCatchRate = data.catchRate || {
    base: 0,
  ...
  if (requestedFields.has(`${resolvedRecursingAs}flavorTexts`) && data.num >= 0) {
  ...
  if (requestedFields.has(`${resolvedRecursingAs}preevolutions`) && data.prevo && data.prevo !== parsingPokemon) {
  ...
  if (requestedFields.has(`${resolvedRecursingAs}evolutions`) && data.evos && data.evos[0] !== parsingPokemon) {
...
__name(mapPokemonDataToPokemonGraphQL, "mapPokemonDataToPokemonGraphQL");
function resolvedNestedAbility(abilityKey, abilityRequestedFields) {
  if (abilityKey) {
...
__name(resolvedNestedAbility, "resolvedNestedAbility");
function resolveNestedType(type, typeRequestedFields) {
  return mapTypesToTypeMatchupGraphQL({ primaryType: toLowerSingleWordCase(type) }, typeRequestedFields);
}
__name(resolveNestedType, "resolveNestedType");
function resolvedNestedLearnset(pokemon, generation, pokemonLearnsetRequestedFields) {
...
__name(resolvedNestedLearnset, "resolvedNestedLearnset");
function parseBaseStatsTotal(baseStats) {
  return baseStats.hp + baseStats.atk + baseStats.def + baseStats.spa + baseStats.spd + baseStats.spe;
}
__name(parseBaseStatsTotal, "parseBaseStatsTotal");
function parseMinimumHatchTimeForMaximumHatchTime(minimumHatchTime) {
  return minimumHatchTime ? minimumHatchTime + 256 : void 0;
}
__name(parseMinimumHatchTimeForMaximumHatchTime, "parseMinimumHatchTimeForMaximumHatchTime");
...
__name(parseSpeciesForBulbapedia, "parseSpeciesForBulbapedia");
function parseSpeciesForSerebiiPage(pokemonName, pokemonNumber, pokemonTier) {
  if (pokemonNumber <= 0)
    return "";
...
__name(parseSpeciesForSerebiiPage, "parseSpeciesForSerebiiPage");
function parseDataForEvolutionRecursion(basePokemonData, _) {
...
__name(parseDataForEvolutionRecursion, "parseDataForEvolutionRecursion");
function parseSpeciesForSmogonPage(pokemonName, pokemonNumber, pokemonTier) {
  if (pokemonNumber === 0)
    return "";
  const parsedPokemonName = toLowerHyphenCase(pokemonName.replace(/:/g, ""));
  if (pokemonNumber < 0) {
...
__name(parseSpeciesForSmogonPage, "parseSpeciesForSmogonPage");
var PokemonReferencedCallIdentifier = /* @__PURE__ */ ((PokemonReferencedCallIdentifier2) => {
  PokemonReferencedCallIdentifier2["None"] = "none";
  PokemonReferencedCallIdentifier2["AbilityToPokemon"] = "abilityToPokemon";
  PokemonReferencedCallIdentifier2["MoveToPokemon"] = "moveToPokemon";
  return PokemonReferencedCallIdentifier2;
})(PokemonReferencedCallIdentifier || {});
export {
  PokemonReferencedCallIdentifier,
  mapPokemonDataToPokemonGraphQL
};
//# sourceMappingURL=pokemonMapper.js.map